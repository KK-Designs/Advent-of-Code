...
calibration.forEach(str => {
	let codeStr = str.replace(/\D/gm, '').split('').map(Number);
	codeStr = codeStr.slice(0, 1).concat(codeStr.slice(-1));
	codeStr = codeStr.length == 1 ? codeStr.flatMap(i => [i, i]) : codeStr;
	codeStr = Number(codeStr.join(''));
	values.push(codeStr);
});
